<script lang="ts">
	...
	import { BUTTON_VARIANT_COLORS_MAP } from '$lib/Button/types.js';
	import { SIZE_REDUCE_MAP } from '../select';
	import { BADGE_SIZE_VARIANTS, type BadgeProps } from './types.js';
...
<span
	bind:this={ref}
	class={`inline-flex items-center gap-1 select-none! ${roundClass} ${BADGE_SIZE_VARIANTS[size].badge} font-medium badge-${size} ${className} ${BUTTON_VARIANT_COLORS_MAP[variant][color]}`}
	{...restProps}
>
	{#if startIcon}
		<Icon icon={startIcon} />
